...
package com.augustro.filemanager.fragments;
...
import com.augustro.filemanager.activities.superclasses.ThemedActivity;
import com.augustro.filemanager.adapters.RecyclerAdapter;
import com.augustro.filemanager.adapters.data.LayoutElementParcelable;
import com.augustro.filemanager.asynchronous.asynctasks.DeleteTask;
import com.augustro.filemanager.asynchronous.asynctasks.LoadFilesListTask;
import com.augustro.filemanager.asynchronous.handlers.FileHandler;
import com.augustro.filemanager.database.CloudHandler;
import com.augustro.filemanager.database.CryptHandler;
import com.augustro.filemanager.database.SortHandler;
import com.augustro.filemanager.database.models.EncryptedEntry;
import com.augustro.filemanager.database.models.Tab;
import com.augustro.filemanager.filesystem.CustomFileObserver;
...
import com.augustro.filemanager.filesystem.MediaStoreHack;
...
import com.augustro.filemanager.ui.views.DividerItemDecoration;
import com.augustro.filemanager.ui.views.FastScroller;
import com.augustro.filemanager.ui.views.RoundedImageView;
import com.augustro.filemanager.ui.views.WarnableTextInputValidator;
import com.augustro.filemanager.ui.views.appbar.BottomBar;
import com.augustro.filemanager.utils.BottomBarButtonPath;
...
import com.augustro.filemanager.utils.SmbStreamer.Streamer;
...
import com.augustro.filemanager.utils.files.EncryptDecryptUtils;
import com.augustro.filemanager.utils.files.FileListSorter;
...
import static com.augustro.filemanager.fragments.preference_fragments.PreferencesConstants.PREFERENCE_DIRECTORY_SORT_MODE;
import static com.augustro.filemanager.fragments.preference_fragments.PreferencesConstants.PREFERENCE_GRID_COLUMNS;
import static com.augustro.filemanager.fragments.preference_fragments.PreferencesConstants.PREFERENCE_SHOW_DIVIDERS;
import static com.augustro.filemanager.fragments.preference_fragments.PreferencesConstants.PREFERENCE_SHOW_GOBACK_BUTTON;
import static com.augustro.filemanager.fragments.preference_fragments.PreferencesConstants.PREFERENCE_SHOW_HIDDENFILES;
import static com.augustro.filemanager.fragments.preference_fragments.PreferencesConstants.PREFERENCE_SHOW_THUMB;
import static com.augustro.filemanager.fragments.preference_fragments.PreferencesConstants.PREFERENCE_TEXTEDITOR_NEWSTACK;
...
//            getMainActivity().setPagingEnabled(true);
    ...
    public void returnIntentResults(HybridFileParcelable baseFile) {
        ...
        if (getMainActivity().mRingtonePickerIntent) {
            ...
            //Log.(getClass().getSimpleName(), mediaStoreUri.toString() + "\t" + MimeTypes.getMimeType(baseFile.getPath(), baseFile.isDirectory()));
    ...
    public void loadlist(final String path, final boolean back, final OpenMode openMode) {
        ...
        loadFilesListTask = new LoadFilesListTask(ma.getActivity(), path, ma, openMode,
            ...
            if (data != null && data.second != null) {
                ...
                BottomBar bottomBar = getMainActivity().getAppbar().getBottomBar();
                if (IS_LIST){
                    bottomBar.toListButton.setVisibility(View.GONE);
                    bottomBar.toGridButton.setVisibility(View.VISIBLE);
                }else {
                    bottomBar.toListButton.setVisibility(View.VISIBLE);
                    bottomBar.toGridButton.setVisibility(View.GONE);
                }
        ...
        if (!results) {
            if (!mRetainSearchTask) {
                ...
                if (MainActivityHelper.SEARCH_TEXT != null) {
                    ...
                    getMainActivity().mainFragment = (MainFragment) getMainActivity().getFragmentAtFrame();
    ...
    public void onResume() {
        ...
        startFileObserver();
        getMainActivity().getAppbar().setTitle("File Manager");
    }
    ...
    public void updatePaths(){
        ((MainActivity)getActivity()).getAppbar().getBottomBar().updatePath(ma.getCurrentPath(), ma.results,
                MainActivityHelper.SEARCH_TEXT, ma.openMode, ma.folder_count, ma.file_count, ma);
        ((MainActivity)getActivity()).getDrawer().selectCorrectDrawerItemForPath(ma.getCurrentPath());
    }
    ...
    public String getCurrentPath() {
        //Log.(TAG, "getCurrentPath: >>"+ CURRENT_PATH);
        return CURRENT_PATH;
    }
