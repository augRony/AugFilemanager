...
package com.augustro.filemanager.utils.files;
...
import com.augustro.filemanager.utils.OnProgressUpdate;
...
import com.augustro.filemanager.utils.share.ShareTask;
...
public class FileUtils {
    ...
    public static int getNumberOfFilesInDirectory(String path) {
        try {
            File f = new File (path);
            if(f != null && f.exists() && f.isDirectory() && canListFiles(f))
            {
    ...
    public static String getNumberOfFilesInDirectoryAsString(Context context, String path) {
        try {
            int numFile = FileUtils.getNumberOfFilesInDirectory(path);
            if(numFile >= 0) {
                String numOfItems = (numFile != 0? numFile + " ":"0 ")
                        + context.getResources().getQuantityString(R.plurals.items, numFile) ;
                return numOfItems;
            }
        } catch (Exception e) {
        }
