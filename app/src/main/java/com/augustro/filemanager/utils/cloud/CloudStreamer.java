...
public class CloudStreamer extends CloudStreamServer {
    ...
    public Response serve(String uri, String method, Properties header, Properties parms, Properties files) {
        ...
        if (inputStream == null)
            ...
            //Log.(CloudUtil.TAG, "Request: " + range + " from: " + startFrom + ", to: " + endAt);
            ...
            if (range != null && startFrom > 0) {
                if (startFrom >= fileLen) {
                    ...
                    if (newLen < 0)
                        newLen = 0;
                    //Log.(CloudUtil.TAG, "start=" + startFrom + ", endAt=" + endAt + ", newLen=" + newLen);
                    final long dataLen = newLen;
                    source.moveTo(startFrom);
                    //Log.(CloudUtil.TAG, "Skipped " + startFrom + " bytes");
